/**
 * State-dependent priority
 * 
 * Comando de compilação: gcc -lm sdp.c ou clang -lm sdp.c
 * */
#include <stdio.h>
#include <stdlib.h>
...
void execute(process* p, double* totalTime, double* totalWaiting, double* totalTTime, int lenQueue){
	p->totalTime += min(MAX_Q/lenQueue, p->execTime);
...
void sdp(process* p, int n){
	...
	while(deadProcess != n){
		...
		if(lenQueue > 0){
			int index = pop(q);
			execute(&p[index], &totalTime, &totalWaiting, &totalTTime, lenQueue);
